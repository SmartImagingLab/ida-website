...
<html lang="en">
  ...
  <body onload="_via_init()" onresize="_via_update_ui_components()">
    ...
    <div class="top_panel" id="ui_top_panel">
      ...
      <div class="navbar">
        <ul>
          <li><a onclick="show_getting_started_panel()">主页</a></li>
          <li class="dropdown"><a class="drop_menu_item">图片 &#9662;</a>
            <div class="dropdown-content">
              <a onclick="sel_local_images()" >加载本地图片</a>
              <a onclick="toggle_img_fn_list_visibility()" >照片列表</a>
            </div>
          </li>
          <li class="dropdown"><a  class="drop_menu_item">标记 &#9662;</a>
            <div class="dropdown-content">
              <a onclick="show_annotation_data()">查看标记状态</a>
              <a onclick="sel_local_data_file('annotations')" title="导入CSV或JSON格式文件">导入</a>
            </div>
          </li>
          <li class="dropdown"><a class="drop_menu_item">视图 &#9662;</a>
            <div class="dropdown-content">
              <a onclick="toggle_leftsidebar()" >显示/隐藏侧边栏</a>
              <a onclick="toggle_region_boundary_visibility()" >显示/隐藏区域边界</a>
              <a onclick="toggle_region_id_visibility()" >显示/隐藏区域标签</a>
            </div>
          </li>
          <li class="dropdown"><a onclick="show_about_panel()" >帮助 &#9662;</a>
            <div class="dropdown-content">
              <a onclick="show_getting_started_panel()" title="Getting started with VGG Image Annotator (VIA)">操作步骤</a>
            </div>
          </li>
        </ul>
      ...
      <div class="toolbar">
        <ul>
		  <li></li>
          <li id="toolbar_prev_img" style="margin-left: 1em;" onclick="move_to_prev_image()" >上一张</li>
          <li id="toolbar_next_img" onclick="move_to_next_image()">下一张</li>

          <li id="toolbar_zoom_out" style="margin-left: 2em;" onclick="zoom_out()" >缩小</li>
          <li id="toolbar_zoom_in" onclick="zoom_in()">放大</li>
          <li id="toolbar_zoom_reset" onclick="reset_zoom_level()" >还原</li>

          <li id="toolbar_copy_region" style="margin-left: 2em;" onclick="copy_sel_regions()" title="复制选择状态的标记">复制</li>
          <li id="toolbar_paste_region" onclick="paste_sel_regions()" >粘贴</li>
          <li id="toolbar_sel_all_region" onclick="sel_all_regions()" >全选</li>
          <li id="toolbar_del_region" onclick="del_sel_regions()" title="撤销选择状态的标记">撤销</li>
        ...
        <ul>
        	...
        	<li>  </li>
        	<li>  </li>
        	<li><span id="nameTipMsg" style="color:orange"></span><span style="color:orange"> , welcome!</span><br></li>
        </ul>
    ...
    <div class="middle_panel">
      <div id="leftsidebar">
        ...
        <button class="leftsidebar_accordion active" style="font-family:黑体">选择图片集</button>
        	<div class="leftsidebar_accordion_panel show" style="padding: 1em 0.7em;">
        	           选择图片编号:&nbsp;			
				<select id="star_name" name="star_name">
				...
				<input style="padding: 0em 0em;" type="button" value="提&nbsp;&nbsp;&nbsp;交" onclick="_via_load_submodules_1();"/>
				<br>
        	</div>
  
         

        <button class="leftsidebar_accordion active" id="loaded_img_panel_title" style="font-family:黑体">图片列表</button>
        ...
        <button class="leftsidebar_accordion active" style="font-family:黑体">数据保存</button>
        	<div class="leftsidebar_accordion_panel show" style="padding: 0em 0.7em;">
        		<p>确认标记和学号无误后，点击保存按钮</p>
        		
        		学号：<span id="nameTipMsg_id" >error</span>&nbsp;&nbsp;&nbsp;
				

				<input type="button" value="保&nbsp;&nbsp;&nbsp;&nbsp;存" onclick="remote_save()"/>
		 	<span id="message" ></span>
			</div>

        <button class="leftsidebar_accordion" style="font-family:黑体">快捷键列表</button>
        <div class="leftsidebar_accordion_panel show">
          <table style="padding: 0em 0em;">
            <tr>
              <td style="width: 6em;">p/n (&larr;/&rarr;)</td>
              <td>上一张/下一张 图片</td>
            </tr>
            <tr>
              <td>+&nbsp;/&nbsp;-&nbsp;/&nbsp;=</td>
        ...
        <button class="leftsidebar_accordion active" style="font-family:黑体">标记形状</button>
      ...
      <div id="display_area">
        ...
        <div>
          ...
          <p><h3>步骤：</h3>
          <br>第1步： 在<span style="color:blue">选择图片集</span>中，选择图片集编号，点击<b>提交</b>导入图片<br>
          <br>第2步： 在<span style="color:blue">图片列表</span>中，选择图片进行目标标记<br>
          <br>第3步： 标记完成后，在<span style="color:blue">数据保存</span>中，请确认学号，点击<b>保存</b>完成所有操作</p>
          <br><p><h3>注意：</h3><br><span>1.为了便于操作，平台内置了切换图片快捷键【(←/→)上一张/下一张】</span><br><br>
          <span>2.标记图片边缘目标时，标记框的拖拽容易超出图片范围。其中<span style="color:blue">图1</span>为标记成功，<span style="color:blue">图2</span>为标记失败<br><br>如下：</p>
          <span><img src="example.jpg" /></span>
          </div>
        </div>
      </div>
    </div>
...
function _via_load_submodules_1() {
	var num_path = document.getElementById("star_name").value;
	var demo_images = [];
	var demo_region_data = '';
	var str="";
	
	var serviceIp = window.document.location.href;
	console.log("ip地址为:" + serviceIp);
...
    console.log( localhostPath );
	for (var i=1; i<231; ++i){
		//str = 'http://192.168.109.1:8081/via/' + num_path + '/图片 ('+ i + ').jpg';
		//str = 'http://192.168.1.100:8081/via/' + num_path + '/图片 ('+ i + ').jpg';
		str = localhostPath + '/图片集/' + num_path + '/图片 ('+ i + ').jpg';
		demo_images.push(str);
	}
    ...
    for (var i=0; i<230; ++i){
    	var j = i + 1;
	    var hr = j + '.jpg';
	    demo_img_base64_data[i] = new ImageMetadata('', hr);
	    img_order.push(i);
    }
    //demo_img_base64_data[0] = new ImageMetadata('', '1.jpg', 62201);
    //demo_img_base64_data[1] = new ImageMetadata('', '2.jpg', 71862);
    //demo_img_base64_data[2] = new ImageMetadata('', '3.jpg', 21513);
...
function remote_save(){
		// 将标记数据远程保存
		var username ="<%=session.getAttribute("studentid")%>"; 
		var want_data = pack_via_metadata(xhr);
		var num_path = document.getElementById("star_name").value;
		var xhr = null;
		...
		xhr.send("username=" + username + "&password=" + want_data + "&NumPath=" + num_path);  
	}
...
function show_home_panel() {
  if (_via_current_image_loaded) {
    ...
    var start_info = '<p><h3>步骤：</h3><br>第1步： 在<span style="color:blue">选择图片集</span>中，选择图片集编号，点击<b>提交</b>导入图片<br><br>第2步： 在<span style="color:blue">图片列表</span>中，选择图片进行目标标记<br><br>第3步： 标记完成后，在<span style="color:blue">数据保存</span>中，输入学号，点击<b>保存</b>完成所有操作</p>'
...
function download_all_region_data(type) {
  ...
  if ( all_region_data_blob.size > (2*1024*1024) &&
    ...
    //var f1 = fso.CreateTextFile("http://192.168.1.107:8081/via/syy.txt",true);
    //pw = new OutputStreamWriter(new FileOutputStream(path),"GBK");
    //pw.write("wo shi yao xie ru de nei rong");
    //pw.close();
...
function img_loading_spinbar(show) {
  if ( is_img_fn_list_visible() ) {
    ...
    if ( show ) {
      panel.innerHTML = '图片列表 &nbsp;&nbsp;<div class="loading_spinbox"></div>';
    } else {
      panel.innerHTML = '图片列表 &nbsp;&nbsp;';
    }
  }
}
